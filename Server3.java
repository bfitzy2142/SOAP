...
@WebService(targetNamespace = "http://localhost", serviceName = "ServiceB", endpointInterface = "ServiceBInterface", portName = "Server3port")

/**
 * @Class Server3: Communicates with the APIGateway to return a service to the
 *        client
 */
public class Server3 implements ServiceBInterface {
	...
	public Server3(APIGateway publisher) {
	...
	public String getServiceB() {
		...
		System.out.println("REQ:" + publisher.getRequestnum() + " " + time.format(new Date()) + "\nClient: '" + host
				+ "' connected with the address '" + remoteAddr + "' on Server3 for ServiceB!");
		message = "This is Server 3, giving you Service B!";
